...
use super::{
    utils,
    vector::{vec2::Vec2, vec4::Vec4, VecMethods},
...
impl OpenSimplexNoise4D {
    fn inside_pentachoron_at_0_0_0_0(
        ...
        // may contribute. This depends on the closest two pentachoron vertices,
        // including (0, 0, 0, 0)
        let u_ins = 1.0 - ins.sum();
        let value = if u_ins > score.x || u_ins > score.y {
    ...
    fn inside_second_dispentachoron(
        ...
        if is_bigger_side.x == is_bigger_side.y {
            if is_bigger_side.x {
                ...
                // Two contributions are permutations of (0, 0, 0, 1) and (0, 0, 0, 2) based on
                // c1
